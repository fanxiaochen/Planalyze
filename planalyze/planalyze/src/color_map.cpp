...
const osg::Vec4& ColorMap::getColor(DiscreteColor map, float value, float low, float high)
{
    switch(map) {
    case(DISCRETE_TOTAL):
      return getColor(_discrete_total, value, low, high);
      break;
    case(DISCRETE_KEY):
      return getColor(_discrete_key, value, low, high);
      break;
    default:
      return getColor(_discrete_total, value, low, high);
      break;
